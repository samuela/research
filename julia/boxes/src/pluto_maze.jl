...
# ╔═╡ bbc449b1-158c-4b50-83f2-9d9b109377ab
...
begin
	...
	import ._mod: Box, BoxSet, in_box, add_box!, rand_inside, intersect_boxes
...
function plot_maze()
	...
	PyPlot.scatter([start[1]], [start[2]], marker = "^", s = 100, color = :darkgreen)
	PyPlot.scatter([goal[1]], [goal[2]], marker = "*", s = 125, color = :darkorange)
	PyPlot.axis("equal")
	PyPlot.xticks([])
	PyPlot.yticks([])
	fig, ax
...
# ╔═╡ 6a647f18-630d-4f5f-a498-4c453f78ea49
function a_star_midpoints(free_boxset, start, goal)
	# id's are either :start, :goal, or (i, j) where i, j index into free_boxset.boxes.
	T = Union{Symbol, Tuple{Int, Int}}
	xys = Dict{T, Array}(:start => start, :goal => goal)
	
	# L2 distance from point with id i to point with id j
	l2(i, j) = sqrt(sum((xys[i] - xys[j]) .^ 2))
	h(i) = l2(i, :goal)

	open_set = Array{T}([:start])
	came_from = Dict()
	g_score = Dict{T, Real}(:start => 0.0)			# best known dist from start to x
	f_score = Dict{T, Real}(:start => h(:start))	# f_score(x) = g_score(x) + h(x)
	
	# x is actually and xy point here.
	containing_boxes(x) = findall(in_box.(free_boxset.boxes, Ref(x)))
	midpoint(b) = 0.5 * (b.mins + b.maxs)
	goal_boxes = containing_boxes(goal)
	all_neighbors(i) = begin
		i_boxes = containing_boxes(xys[i])
		
		# These are only those points that are intersections of two boxes, never the goal.
		pts = [(min(b1, b2), max(b1, b2)) for b1 in i_boxes for b2 in free_boxset.adjacency[b1]]
		# Ensure that all pts have corresponding xys values.
		for (bi, bj) in pts
			xys[(bi, bj)] = midpoint(intersect_boxes(free_boxset.boxes[bi], free_boxset.boxes[bj]))
		end
		
		if isempty(intersect(i_boxes, goal_boxes))
			pts
		else
			[:goal, pts...]
		end
	...
	while !isempty(open_set)
		# will x always be in f_score?
		ix = argmin([f_score[x] for x in open_set])
		current = open_set[ix]
		deleteat!(open_set, ix)
		
		if current == :goal
			# Found the goal!
			path = Array{T}([:goal])
			while path[end] != :start
				push!(path, came_from[path[end]])
			end
			return [xys[i] for i in path]
		end
		
		for neighbor in all_neighbors(current)
			tentative_g_score = g_score[current] + l2(current, neighbor)
	...
	# Could not find a reachable path, implicitly return `nothing`.
end
...
begin
	...
	figs = map(1:27) do i
		...
		_, _ax = plot_maze()
		PyPlot.title("$i function evaluation(s)")
		...
		path = a_star_midpoints(free_boxset, start, goal)
		if !isnothing(path)
			PyPlot.plot([x for (x, y) in path], [y for (x, y) in path])
		end
...
Mazes = "2e5c213e-ab15-5c8d-991f-abb7b09c23af"
...
git-tree-sha1 = "1e6753e36005e4773c038f99bee97a1bf36dc2e4"
...
[[Mazes]]
deps = ["Plots", "SimpleDrawing", "SimpleGraphs", "SimplePartitions"]
git-tree-sha1 = "4a8a368fffb4d31d9a2a07a4916e3cc33e150370"
...
git-tree-sha1 = "110339ca39489ce448fbc21f141c00496fb416d5"
...
[[SimpleDrawing]]
deps = ["LinearAlgebra", "Plots"]
git-tree-sha1 = "b7e1faba849cb46d5e0897f1f92c4ad5960364b3"
uuid = "d78a06e8-ae74-583c-9a07-0d6572347000"
version = "0.2.7"

[[SimpleGraphs]]
deps = ["AbstractLattices", "Combinatorics", "DataStructures", "IterTools", "LightXML", "LinearAlgebra", "LinearAlgebraX", "Optim", "Primes", "Random", "RingLists", "SimplePartitions", "SimplePolynomials", "SimpleRandom", "SparseArrays", "Statistics"]
git-tree-sha1 = "ea3510c257a23fb31e128cce9cb7f2ae43d29453"
...
git-tree-sha1 = "ce0389142f3ec4681b0f6bf75da5acd4c43545ef"
