...
--width-multiplier=256 --model-a= --model-b=
...
from mnist_vgg16_run import make_vgg_width_ablation
...
def main():
  ...
  with wandb.init(
    ...
    fig = plot_interp_loss(config.epoch, lambdas, train_loss_interp_naive, test_loss_interp_naive,
                           train_loss_interp_clever, test_loss_interp_clever)
    plt.savefig(f"cifar10_vgg16_weight_matching_interp_loss_epoch{config.epoch}.png", dpi=300)
    ...
    fig = plot_interp_acc(config.epoch, lambdas, train_acc_interp_naive, test_acc_interp_naive,
                          train_acc_interp_clever, test_acc_interp_clever)
    plt.savefig(f"cifar10_vgg16_weight_matching_interp_accuracy_epoch{config.epoch}.png", dpi=300)
