...
def policy_cost_and_grad(dynamics_fn, cost_fn, policy, gamma=1.0):
  ...
  def value_and_grad(policy_params, x0, total_time):
    ...
    primals, vjp = ode.vjp_odeint(ofunc, y0, t, policy_params, rtol=1e-4, mxstep=1e6)
...
def main():
  ...
  def eval_policy(policy_fn, policy_params):
    """Return a function evaluting the policy's cost starting from some x0."""
    def eval_one(x0):
      c, _ = policy_cost_and_grad(dynamics_fn, cost_fn, policy_fn)(policy_params, x0, total_secs)
      return c
  ...
  opt_all_costs = vmap(eval_policy(lambda KK, x: -KK @ x, K))(x0_eval)
  ...
  cost_and_grad = jit(policy_cost_and_grad(dynamics_fn, cost_fn, policy, gamma))
  ...
  est_all_costs = vmap(eval_policy(policy, opt.value))(x0_eval)
